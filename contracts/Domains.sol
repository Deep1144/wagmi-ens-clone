...
contract Domains is ERC721URIStorage {
    ...
    function register(string calldata _domain) public payable {
        if (domains[_domain] != address(0)) revert AlreadyRegistered();
        if (!valid(_domain)) revert InvalidName(_domain);
        uint256 _price = price(_domain);
        // Check if enough Matic was paid in the transaction
        require(msg.value >= _price, "Not enough Matic paid");

        string memory _name = string(abi.encodePacked(_domain, ".", tld));
        string memory finalSvg = string(
            abi.encodePacked(svgPartOne, _name, svgPartTwo)
        );
        uint256 newRecordId = _tokenIds.current();
        uint256 len = StringUtils.strlen(_domain);
    ...
    function setRecord(string calldata _domain, string calldata _record)
        public
    {
        if (msg.sender != domains[_domain]) revert Unauthorized();
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 0);
        if (len == 3) {
            return 0.05 ether; // 5 MATIC = 5 000 000 000 000 000 000 (18 decimals). We're going with 0.5 Matic cause the faucets don't give a lot
        } else if (len == 4) {
            return 0.03 ether; // To charge smaller amounts, reduce the decimals. This is 0.3
        } else {
            return 0.01 ether;
        }
    }
